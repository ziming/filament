...
If you'd like to set the max width for pages of the type `SimplePage`, like login and registration pages, you may do so by using the `simplePageMaxWidth()` method. The default is `Large`:
...
public function panel(Panel $panel): Panel
{
    return $panel
        // ...
        ->simplePageMaxWidth(MaxWidth::Small);
}
```
